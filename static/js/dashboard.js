...
(function () {
    ...
    window.__DETAS_MAP_FORCE_ZOOM__ = true;

    // Haritanın direkt açılacağı konum
    // Antalya / Muratpaşa merkez
    const DETAS_MAP_LAT = 36.8841;
    const DETAS_MAP_LNG = 30.7056;
    const DETAS_MAP_ZOOM = 15;

    function isDetasMap(mapObj) {
        return (
            mapObj &&
            mapObj._container &&
            mapObj._container.id === "map"
        );
    }

    function patchLeafletWhenReady() {
        if (!window.L || !L.Map || !L.Map.prototype) {
            setTimeout(patchLeafletWhenReady, 100);
            return;
        }

        if (L.Map.prototype.__detasForceZoomPatched) return;
        L.Map.prototype.__detasForceZoomPatched = true;
    ...
    patchLeafletWhenReady();

    // Sayfa açıldıktan sonra da haritayı bir kez daha toparla
